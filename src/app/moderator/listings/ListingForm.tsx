...
export default function ListingForm({
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    if (!r.ok) { setError(json?.error ?? (mode === 'edit' ? 'Could not save changes.' : 'Could not add harvest.')); return }
    ...
    return (
      <div className="bg-white rounded-2xl border border-gray-100 p-6 max-w-md">
        ...
        <div className="flex flex-col gap-2 mt-5">
          <button
            onClick={() => { setDone(false); setFarmerId(''); setEmoji('📦'); setForm(EMPTY_LISTING_FORM) }}
