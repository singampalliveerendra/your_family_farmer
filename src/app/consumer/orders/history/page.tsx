...
export default function ConsumerOrderHistoryPage() {
  ...
  const refresh = useCallback(async () => {
    ...
    if (!r) { setError(L('Could not load orders. Check your connection.', 'ఆర్డర్లు లోడ్ కాలేదు. మీ కనెక్షన్ చూడండి.')); setLoading(false); return }
    const json = await r.json().catch(() => ({}))
    if (!r.ok) { setError(json?.error ?? L('Could not load orders.', 'ఆర్డర్లు లోడ్ కాలేదు.')); setLoading(false); return }
