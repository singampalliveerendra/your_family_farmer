...
export default function StickyBottomBar({ farmer }: { farmer: Record<string, unknown> }) {
  const { tx, L } = useLang()
  const f = farmer as Farmer
  const phone = f.phone?.replace(/\D/g, '') ?? ''
  const message = tx.whatsappMessage.replace('{name}', f.name ?? '')
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 z-50 bg-white border-t border-gray-200 px-4 py-3 flex gap-3">
      <a
        ...
        className="flex-1 flex items-center justify-center gap-2 bg-green-50 border-2 border-green-600 text-green-700 font-bold py-4 rounded-xl text-sm"
      >
        ...
        <span>{L('WhatsApp', 'వాట్సాప్')}</span>
      </a>

      <button
        onClick={() => {
          const tabs = document.querySelectorAll('button[class*="border-b-2"]')
          if (tabs[1]) (tabs[1] as HTMLButtonElement).click()
