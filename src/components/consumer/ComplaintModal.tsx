...
const TYPE_OPTIONS = (L: (en: string, te: string) => string) => [
  { value: 'quality_complaint', label: L('Quality problem', 'నాణ్యత సమస్య') },
  { value: 'delivery_delay', label: L('Delivery delay', 'డెలివరీ ఆలస్యం') },
  { value: 'payment_issue', label: L('Payment / refund issue', 'చెల్లింపు సమస్య') },
    ...
    <div className="fixed inset-0 z-50 bg-black/40 flex items-end sm:items-center justify-center p-4" onClick={onClose}>
      <div className="bg-white rounded-2xl w-full max-w-md p-5" onClick={(e) => e.stopPropagation()}>
        ...
        <textarea
          ...
          rows={4}
          autoFocus
          placeholder={L('e.g. Tomatoes arrived spoiled', 'ఉదా: టమోటాలు పాడైపోయాయి')}
          className="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm mb-3 focus:outline-none focus:border-green-500"
        />

        <label className="block text-xs font-bold text-gray-500 mb-1">
          {L('Order code', 'ఆర్డర్ కోడ్')} <span className="font-normal text-gray-300">{L('(optional)', '(ఐచ్ఛికం)')}</span>
        </label>
        <input
          value={orderCode}
