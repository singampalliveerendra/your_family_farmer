...
describe('computeDeliveryTotal', () => {
  // The pricing rule itself: Rs30 for the first farmer and Rs15 for each one
  // after, so 1 farmer is Rs30, 2 is Rs45, 3 is Rs60.
  ...
  // No farmers, or a garbage count, costs nothing instead of NaN.
  ...
  // The total is always a whole rupee amount and never below zero.
  ...
  // Pins the fallback charges used before a moderator sets their own.
...
describe('resolveBatchDeliveryFee', () => {
  ...
  // Nobody is delivering, so there is nothing to charge.
  it('charges nothing for a pure self-pickup checkout', () => {
    expect(inputs()).toBe(0)
  })

  // The first farmer in a home-delivery checkout carries the Rs30 base.
  it('charges the base on the opening batch of a home delivery', () => {
    expect(inputs({ batchHomeDelivery: true })).toBe(30)
  })

  // Each further farmer in the same checkout adds only the Rs15 extra, however
  ...
  // The security case: the browser saying 'no delivery charge' is overruled by
  // our own order rows, whether the delivery is in this batch or a sibling.
  ...
  // The flag is still honoured in the one direction that cannot be abused.
  ...
  // A lone order cannot pretend to be a later batch to pay Rs15 instead of
  // Rs30.
  ...
  // Three farmers posted separately stamp 30 + 15 + 15, which is exactly the
  // whole-checkout total. The buyer is neither over- nor under-charged.
...
describe('planDeliveryRefund', () => {
  // One farmer of two cancels: the buyer gets the Rs15 extra unit back, taken
  // off that farmer's own payment.
  ...
  // Once the final farmer cancels there is nothing left to deliver, so every
  // delivery rupee still held comes back.
  ...
  // Across two cancellations the buyer ends up with the full Rs45, including
  // the part still sitting on the row that cancelled first.
  ...
  // A farmer with two orders who cancels one is still delivering, so no
  // delivery refund is owed.
  ...
  // The buyer overpaid, so the money comes off whichever row actually captured
  // it. The refund follows the money, not the cancellation.
  ...
  // from the sibling that still holds the money.
  ...
  // An unknown order id, or an empty checkout, plans no refund instead of
  // erroring.
  ...
  // End to end: all three farmers cancel one by one and the buyer gets back
  // exactly the Rs60 that was charged, no row ever over-refunded.
